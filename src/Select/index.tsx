import React from 'react';
import styled from 'styled-components';

import { gray300, gray50, gray500, gray800, redError, white } from '../Colors';

const StyledSelect = styled.select`
  border-radius: 2px;
  background-color: ${white} !important;
  border: solid 1px ${gray300};
  width: 100%;
  padding: 12px 16px;
  height: 46px;
  font-size: 14px;
  appearance: none;
  background: url('data:image/png;base64,');
  background-repeat: no-repeat;
  background-position: right 16px center;
  background-size: 18px;
  color: ${props => props.color};
  &:invalid {
    color: ${gray500};
  }
  &:focus {
    outline: none;
    box-shadow: none;
  }
  &:disabled,
  &.disabled {
    background-color: ${gray50};
    &:hover {
      cursor: not-allowed;
    }
  }
  &:focus,
  &.focus {
    border-color: ${gray800};
  }
  &.input-error {
    border: solid 1px ${redError};
    background: url('/images/ic-danger-red.png');
    background-repeat: no-repeat;
    background-position: right 16px center;
    background-size: 18px;
  }
`;

type OptionProps = string | { label?: string; disabled?: boolean; value?: string | number | undefined };

export interface SelectProps extends React.InputHTMLAttributes<HTMLSelectElement> {
  options?: OptionProps[];
}

export default class Select extends React.PureComponent<SelectProps> {
  public render() {
    const { value, placeholder, children, options, ...restProps } = this.props;

    return (
      <StyledSelect value={value} {...restProps} color={value === '' ? gray500 : gray800}>
        {placeholder ? (
          <option value="" hidden disabled>
            {placeholder}
          </option>
        ) : null}
        {children || (options && options.map(this.renderOptions))}
      </StyledSelect>
    );
  }

  private renderOptions = (o: OptionProps, i: number) => {
    if (typeof o === 'string') {
      return (
        <option key={i} value={o}>
          {o}
        </option>
      );
    }
    return (
      <option disabled={o.disabled || false} key={i} value={o.value}>
        {o.label}
      </option>
    );
  };
}
