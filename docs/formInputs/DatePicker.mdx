...
- 범위 선택은 `onRangeChange`와 `rangeValue`를 사용합니다.
...
<Playground>
  ...
  <DatePicker
    ...
    onRangeChange={(start, end) => {
      alert('Changed ' + start.toLocaleString() + ' > ' + end.toLocaleString());
    }}
    alwaysShow
    useRange
    inline
...
`min`과 `max`를 통해 최소, 최대기간을 설정할 수 있습니다.

<Playground>
  <DatePicker min={new Date(2019, 0, 1)} value={new Date(2019, 3, 1)} max={new Date(2019, 11, 31)} alwaysShow inline />
  <DatePicker min={new Date(2020, 4, 1)} max={new Date(2020, 6, 30)} alwaysShow useRange inline />
</Playground>
